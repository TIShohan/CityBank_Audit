...
# --- PAGE CONFIG ---
st.set_page_config(page_title="City Bank Video Reviewer (Mac)", page_icon="🎥", layout="wide")
...
if 'base_path' not in st.session_state:
    # macOS friendly default path
    st.session_state.base_path = os.path.expanduser("~/Downloads/City Bank_Downloads")
...
def select_folder_mac():
    """Uses AppleScript to select a folder (much more stable on Mac than Tkinter)."""
    try:
        # This calls a small AppleScript that opens a native folder picker
        cmd = 'osascript -e "POSIX path of (choose folder with prompt \\"Select City Bank Downloads Folder\\")"'
    ...
    if cand_id:
        ...
        if os.path.exists(v1_full_path) and os.path.exists(v2_full_path):
            ...
            <script>
                ...
                vol1.addEventListener('input', () => {{ v1.volume = vol1.value; volLab1.textContent = Math.round(vol1.value * 100) + '%'; }});
                vol2.addEventListener('input', () => {{ v2.volume = vol2.value; volLab2.textContent = Math.round(vol2.value * 100) + '%'; }});
                ...
                mute1.addEventListener('click', () => {{ v1.muted = !v1.muted; mute1.textContent = v1.muted ? 'Unmute' : 'Mute'; mute1.style.background = v1.muted ? '#475569' : '#e94560'; }});
                mute2.addEventListener('click', () => {{ v2.muted = !v2.muted; mute2.textContent = v2.muted ? 'Unmute' : 'Mute'; mute2.style.background = v2.muted ? '#475569' : '#e94560'; }});

                back5.addEventListener('click', () => {{ debouncedSeek(-5); }});
                fwd5.addEventListener('click', () => {{ debouncedSeek(5); }});

                rate.addEventListener('change', () => {{ v1.playbackRate = parseFloat(rate.value); v2.playbackRate = parseFloat(rate.value); }});
            </script>
            """
